          ...
          >
            <div class="session-info">
              <div class="patient-basic">
                <el-avatar :size="50" :src="session.avatarUrl"></el-avatar>
                <div class="patient-name">
                  <div class="patient-tags">
                    <el-tag size="mini">{{ session.username }}</el-tag>
                    <el-tag size="mini" type="info">{{ session.gender === 'male' ? '男' : '女' }}</el-tag>
                  </div>
                </div>
                <el-badge 
...
export default {
  ...
  watch: {
    sessionType: {
      handler(newType) {
        ...
        this.fetchSessions(this.doctorId,null, null, form)
      },
      immediate: true
    }
  },
  ...
  methods: {
    ...
    async fetchSessions(userId = null, appointmentId = null, form = null) {
      this.loading = true
      try {
        const params = {
    ...
    enterChat(session) {
      ...
      this.$router.push({
        path: '/doctor/consultations/sessions/chat',
    ...
    viewRecord(session) {
      ...
      this.$router.push('/doctor/consultations/records')
    }
  }
}
</script>
