          ...
          <template slot-scope="scope">
            <div class="action-buttons">
              ...
              <template v-if="scope.row.status === 'pending'">
                <el-tooltip content="通过预约" placement="top">
                  <el-button
                    circle
                    size="mini"
                    type="success"
                ...
                <el-tooltip content="拒绝预约" placement="top">
                  <el-button
                    circle
                    size="mini"
                    type="danger"
...
<script>
import { getAppointments, updateAppointment, getAppointmentDetail } from '@/api/appointment'
...
export default {
  ...
  methods: {
    async fetchAppointments() {
      this.loading = true
      try {
        const { data } = await getAppointments(null, this.doctorId, this.searchStatus)
    ...
    async handleApprove(row) {
      try {
        await updateAppointment(row.id, 'confirmed')
    ...
    async handleReject(row) {
      try {
        await updateAppointment(row.id, 'cancelled')
