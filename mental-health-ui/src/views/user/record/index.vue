        ...
        <el-table-column label="医生信息" width="200">
          <template slot-scope="scope">
            <div class="patient-info">
              <el-avatar :size="40" :src="scope.row.doctorAvatar"></el-avatar>
              <div class="patient-detail">
                <span class="patient-name">{{ scope.row.doctorName }}</span>
              </div>
            </div>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchRecords() {
        this.loading = true
        try {
          const res = await getConsultationList({ userId: this.userId })
          if (res.status === '200') {
